...
/**
 * Lecteur YouTube embarqué robuste :
 * - essaie youtube.com puis youtube-nocookie.com en repli ;
 * - détecte les erreurs du player IFrame (100/101/150/152…) ;
 * - affiche un état propre « vidéo bloquée » avec ouverture dans YouTube.
 */
export function YouTubeSurface({ videoId, autoPlay = false }: Props) {
  ...
  if (blocked) {
    return (
      <View style={styles.blocked}>
        ...
        <Text style={styles.blockedTitle}>Vidéo bloquée par YouTube ici</Text>
  ...
  return (
    <WebView
      ...
      onError={fail}
      onMessage={(event) => {
        if (event.nativeEvent.data.startsWith('err:')) fail();
      }}
    />
  );
}
...
const styles = StyleSheet.create({
  ...
  blocked: { flex: 1, alignItems: 'center', justifyContent: 'center', gap: 8, backgroundColor: '#0B0F18', padding: 12 },
